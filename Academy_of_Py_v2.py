...
# load files
school_data_load = r'schools_complete.csv'
student_data_load = r'students_complete.csv'
...
school_data_summary = pd.merge(student_data, school_data, how = "left", on = ["school_name", "school_name"])

# %%
# Calculate the Totals (Schools and Students)
school_count = len(school_data_summary["school_name"].unique())
student_count = school_data_summary["Student ID"].count()

# Calculate the Total Budget
total_budget = school_data["budget"].sum()

# Calculate the Average Scores
average_math_score = school_data_summary["math_score"].mean()
average_reading_score = school_data_summary["reading_score"].mean()
overall_passing_rate = (average_math_score + average_reading_score) / 2

# Calculate the Percentage Pass Rates
passing_math_count = school_data_summary[(school_data_summary["math_score"] >= 70)].count()["student_name"]
passing_math_percentage = passing_math_count / float(student_count) * 100
passing_reading_count = school_data_summary[(school_data_summary["reading_score"] >= 70)].count()["student_name"]
...
per_school_counts = school_data_summary["school_name"].value_counts()

# Calculate the total school budget and per capita spending
# per_school_budget = school_data_complete.groupby(["school_name"]).mean()["budget"]
per_school_budget = school_data_summary.groupby(["school_name"]).mean()["budget"]
per_school_capita = per_school_budget / per_school_counts

# Calculate the average test scores
per_school_math = school_data_summary.groupby(["school_name"]).mean()["math_score"]
per_school_reading = school_data_summary.groupby(["school_name"]).mean()["reading_score"]

# Calculate the passing scores by creating a filtered data frame
school_passing_math = school_data_summary[(school_data_summary["math_score"] >= 70)]
school_passing_reading = school_data_summary[(school_data_summary["reading_score"] >= 70)]
...
ninth_graders = school_data_summary[(school_data_summary["grade"] == "9th")]
tenth_graders = school_data_summary[(school_data_summary["grade"] == "10th")]
eleventh_graders = school_data_summary[(school_data_summary["grade"] == "11th")]
twelfth_graders = school_data_summary[(school_data_summary["grade"] == "12th")]
